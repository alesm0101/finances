...
var MyModelCollection = require('../models/user'); // the name 'users" of the collection is commning from user.js -> mongoose.model('users', schema)
...
router.post('/', function(req, res, next) {
    MyModelCollection.create(req.body, function (err, post) { // create the user
        if (err) return next(err);
        res.json(post);
    });
});
